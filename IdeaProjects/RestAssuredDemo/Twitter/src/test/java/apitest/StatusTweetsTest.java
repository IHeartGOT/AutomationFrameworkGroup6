...
import clint.base.BaseTwitterClient;
import clint.tweets.TweetsClient;
...
public class StatusTweetsTest {
    ...
    // You want to verify in test class level
    @Test
    ...
    public void testPostTweet() throws Exception {
        String tweet = "Had coffee three times today";
        Response response = this.tweetsClient.createTweet(tweet);
        response.then().statusCode(HttpStatus.SC_OK);
    }

    // Here is an example of using the same method "createTweet()"
    // but for a different test scenario
    // User cannot tweet the same tweet twice so we are basically breaking
    // this in two steps
    // Step 1: Make sure user can create a tweet (ex: tweet = "Hello")
    // Step 2: Verify user was able to create the tweet (Status 200)
    // Step 3: Attempt to tweet the same thing again
    // Step 4: Verify user was UNABLE to create the tweet (Status 403)
    @Test
